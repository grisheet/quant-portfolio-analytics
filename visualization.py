...
# ---------------------------------------------------------------------------
# 1. Cumulative performance — "did the portfolio beat the benchmark?"
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 2. Drawdown — "how painful were the losses?"
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3. Rolling volatility — "how has risk changed over time?"
# ---------------------------------------------------------------------------

def plot_rolling_volatility(port_rets: pd.Series, bench_rets: pd.Series | None,
                            window: int = 63) -> plt.Figure:
    fig, ax = plt.subplots(figsize=(11, 4.5))
    m.rolling_volatility(port_rets, window).plot(ax=ax, color=ACCENT, lw=1.8,
                                                 label="Portfolio")
    if bench_rets is not None:
...
# ---------------------------------------------------------------------------
# 4. Rolling Sharpe — "is risk-adjusted performance consistent?"
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5. Correlation heatmap — "is the portfolio actually diversified?"
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 6. Return distribution — "how fat are the tails?"
# ---------------------------------------------------------------------------
...
def plot_return_distribution(port_rets: pd.Series, confidence: float = 0.95) -> plt.Figure:
    ...
    ax.text(-var, ax.get_ylim()[1] * 0.9, f"  VaR {confidence:.0%}: {var:.2%}",
            color=NEGATIVE, fontsize=10)
...
# ---------------------------------------------------------------------------
# 7. Risk-return scatter with Monte Carlo frontier
# ---------------------------------------------------------------------------
...
def plot_efficient_frontier(frontier: pd.DataFrame, current: dict[str, float],
    ...
    ax.scatter(ms["volatility"], ms["return"], marker="*", s=340, color="#e3b341",
               edgecolor="black", zorder=5, label="Max Sharpe")
    ax.scatter(mv["volatility"], mv["return"], marker="D", s=110, color="#2da44e",
               edgecolor="black", zorder=5, label="Min Volatility")
...
# ---------------------------------------------------------------------------
# 8. Monthly returns heatmap — "seasonality and streaks at a glance"
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 9. Allocation drift — "why rebalancing matters"
# ---------------------------------------------------------------------------

def plot_weight_drift(drift: pd.DataFrame) -> plt.Figure:
    fig, ax = plt.subplots(figsize=(11, 5))
    ax.stackplot(drift.index, drift.T.values, labels=drift.columns,
                 colors=sns.color_palette("crest", n_colors=drift.shape[1]), alpha=0.9)
    ax.set_title("Buy-and-Hold Allocation Drift")
    ax.set_ylabel("Portfolio weight")
    ax.set_xlabel("")
    ax.set_ylim(0, 1)
    _pct_axis(ax)
    ax.legend(loc="upper left", frameon=False, ncol=min(4, drift.shape[1]), fontsize=9)
